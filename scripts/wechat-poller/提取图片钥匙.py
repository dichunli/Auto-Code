...
移植自开源项目 328336690/wechat-decrypt（Apache-2.0），已适配本工具：
  - 微信数据目录从 ~/.wechat-cli/config.json 读取（即 wechat-cli init 时用的账号）
  - 抓到的钥匙保存到本脚本同目录的 image_key.json（poller2.py 读取它解密图片）

用法：运行此脚本后，在微信里点开查看几张图片（看大图），
脚本会自动检测到 key 并保存。按 Ctrl+C 退出。
"""
import os
import sys
import re
...
def main():
    # 微信数据目录读 wechat-cli 的配置（init 时选定的账号）
    wechat_cli配置路径 = os.path.join(os.path.expanduser("~"), ".wechat-cli", "config.json")
    with open(wechat_cli配置路径, encoding="utf-8") as f:
        db_dir = json.load(f)['db_dir']
    base_dir = os.path.dirname(db_dir)
    attach_dir = os.path.join(base_dir, 'msg', 'attach')

    # 钥匙存到本脚本同目录的 image_key.json（poller2.py 读这个文件解密图片）
    钥匙路径 = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'image_key.json')
    config = {}
    if os.path.exists(钥匙路径):
        with open(钥匙路径, encoding="utf-8") as f:
            config = json.load(f)
    ...
    try:
        ...
        while True:
            ...
            if aes_key:
                ...
                with open(钥匙路径, 'w', encoding="utf-8") as f:
                    json.dump(config, f, indent=2, ensure_ascii=False)
                print(f"Saved to {钥匙路径}", flush=True)
