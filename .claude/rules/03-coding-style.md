...
- **列表页、看板、报表等数据展示页面的首屏数据查询必须在服务端（Server Component）完成，禁止在 Client Component 的 `useEffect` 中直接进行首屏数据加载**，避免 SPA 导航时 session 未就绪导致页面空白；后续搜索、翻页、筛选等交互可调用 Server Action 或在已获取的数据上做前端过滤，视数据量和实时性要求选择
...
- 搜索功能统一使用 **`useDebounce` Hook**（`src/lib/useDebounce.ts`），禁止手写 `setTimeout` + `clearTimeout` 防抖
  - 正确用法：`const debouncedQuery = useDebounce(query, 300);`
  - 搜索逻辑放在 `useEffect(() => { doSearch(debouncedQuery); }, [debouncedQuery]);` 中
  - 输入框 `onChange` 只更新原始状态：`setQuery(e.target.value)`
