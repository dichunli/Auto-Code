...
export function useUpload(选项: 上传选项 = {}): UseUploadReturn {
  ...
  /* 校验单个文件 */
  const 校验文件 = useCallback(
    async (file: File): Promise<校验结果> => {
      const 类型 = mediaType;
    ...
    [mediaType, maxFileSizeMB, maxDurationSeconds]
  );

  /* 处理图片：压缩 */
  const 处理图片 = useCallback(
    ...
    [compressMaxDimension, compressMaxKB]
  );

  /* 上传单个文件（fetch 方式，用于图片） */
  const 上传图片 = useCallback(
  ...
  const 上传视频 = useCallback(
    (file: File | Blob, fileName: string): Promise<string> => {
  ...
  const 上传 = useCallback(
    async (files: FileList | File[]): Promise<上传结果> => {
      ...
      取消控制器.current = null;
      set上传中(false);
      set进度((进) => (total > 0 && urls.length === total ? 100 : 进));

      return { urls, errors };
    },
    [校验文件, 处理图片, 上传视频, 上传图片, folder, onProgress, onSuccess]
  );
