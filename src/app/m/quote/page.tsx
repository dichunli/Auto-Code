  ...
  return (
    <div className="flex flex-col h-full">
      <div className="sticky top-0 z-40 bg-white border-b border-gray-200 flex items-center justify-between px-4 h-12 shrink-0">
        <h1 className="text-base font-semibold text-gray-900">询价报价</h1>
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-500">{(orders?.length ?? 0)} 单</span>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs px-2 py-0.5 rounded bg-blue-50 text-blue-700">
                  {order.status === "pending_diagnosis" ? "待诊断" : order.status === "pending_repair" ? "待维修" : "维修中"}
                </span>
              </div>
              ...
              <div className="text-xs text-gray-400 flex items-center justify-between">
                <span>工单: {order.order_no}</span>
                <span className="text-orange-600 font-medium">预估 ¥{order.total_cost || 0}</span>
              </div>
            </Link>
          ))
        ) : (
