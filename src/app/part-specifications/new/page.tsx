...
export default function NewPartSpecificationPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("part_specifications").insert({ name });
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 max-w-2xl">
        <div>
          ...
          <input required className="w-full px-3 py-2 border border-gray-300 rounded-lg" placeholder="如：5W-30 1L、D1109" value={name} onChange={(e) => setName(e.target.value)} />
        </div>
