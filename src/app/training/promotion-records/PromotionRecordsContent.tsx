...
export default function PromotionRecordsContent({ initialRecords }: { initialRecords: 晋级记录[] }) {
  const supabase = useMemo(() => createClient(), []);
  /* 首屏数据由服务端传入；loading 仅用于审核操作后的客户端重查 */
  const [records, setRecords] = useState<晋级记录[]>(initialRecords);
