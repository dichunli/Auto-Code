...
export default async function PickingOrderDetailPage({
  params,
}: {
  ...
  const { data: order } = await supabase
    .from("picking_orders")
    .select(
      "id, picking_no, status, total_quantity, receiver_name, notes, created_at, work_orders(id, order_no, vehicles(plate_number), customers(name)), profiles(full_name)"
    )
    .eq("id", id)
    .single();
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="hidden print:block text-center mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">{领料单.picking_no}</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-6 print:border-black print:rounded-none">
        <div className="px-6 py-4 border-b border-gray-100 print:border-black">
          <h1 className="text-lg font-bold text-gray-900 print:hidden">领料单详情</h1>
        </div>
        <div className="px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <div className="text-xs text-gray-500">领料单号</div>
            <div className="font-medium text-gray-900">{领料单.picking_no}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="font-medium text-gray-900">
              {领料单.work_orders?.vehicles?.plate_number || "-"}
            </div>
          </div>
          <div>
            <div className="text-xs text-gray-500">客户</div>
            <div className="font-medium text-gray-900">
              {领料单.work_orders?.customers?.name || "-"}
            </div>
          </div>
          <div>
            <div className="text-xs text-gray-500">总数量</div>
            <div className="font-medium text-gray-900">{领料单.total_quantity}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="font-medium text-gray-900">{领料单.profiles?.full_name || "-"}</div>
          </div>
          <div>
            <div className="text-xs text-gray-500">日期</div>
            <div className="font-medium text-gray-900">
              {new Date(领料单.created_at).toLocaleString("zh-CN")}
            </div>
          </div>
          {领料单.notes && (
            <div className="col-span-2 md:col-span-4">
              <div className="text-xs text-gray-500">备注</div>
              <div className="font-medium text-gray-900">{领料单.notes}</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden print:border-black print:rounded-none">
        <div className="px-6 py-3 border-b border-gray-100 bg-gray-50 print:bg-white print:border-black">
          <h3 className="text-sm font-semibold text-gray-900">领料明细</h3>
        </div>
      ...
      <div className="hidden print:block mt-8 pt-8 border-t border-black">
        <div className="grid grid-cols-3 gap-8 text-sm">
          <div>
            <div className="text-gray-500 mb-8">发料人签字：</div>
            <div className="border-b border-gray-400 h-6"></div>
          </div>
          <div>
            <div className="text-gray-500 mb-8">领料人签字：{领料单.receiver_name || ""}</div>
            <div className="border-b border-gray-400 h-6"></div>
          </div>
          <div>
