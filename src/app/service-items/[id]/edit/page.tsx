...
export default function EditServiceItemPage() {
  ...
  interface CompanyResult { id: string; name: string }
  ...
  const [spCompanyResults, setSpCompanyResults] = useState<CompanyResult[]>([]);
  const [spCustomerQuery, setSpCustomerQuery] = useState("");
  const [spCustomerResults, setSpCustomerResults] = useState<CustomerResult[]>([]);
  const [spVehicleQuery, setSpVehicleQuery] = useState("");
  const [spVehicleResults, setSpVehicleResults] = useState<VehicleResult[]>([]);
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      ...
      setSpVehicleResults((data as VehicleResult[]) || []);
    }, 300);
    return () => clearTimeout(t);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setVehiclePrices(((vehicleData || []) as unknown as VehicleDataRow[]).map((v) => {
          const m = v.vehicle_models;
          const parts: (string | number)[] = [m?.品牌, m?.车系, m?.车型].filter(Boolean) as (string | number)[];
        ...
        setSpecialPrices(((specialData || []) as unknown as SpecialDataRow[]).map((s) => ({
  ...
  function formatVehicleName(m: VehicleModelInfo | null): string {
    if (!m) return "";
    const parts: string[] = [m.品牌, m.车系, m.车型].filter((x): x is string => !!x);
    if (m.年款) parts.push(`${m.年款}款`);
    if (m.排量) parts.push(m.排量);
    return parts.join(" ");
  }
  ...
  return (
    <div>
      ...
      <VehiclePriceEditModal
        ...
        prices={vehiclePrices as unknown as ComponentProps<typeof VehiclePriceEditModal>["prices"]}
      ...
      <VehicleDeleteModal
        ...
        vehicles={(deleteGroupKey ? vehiclePrices.filter((p) => makeGroupKey(p) === deleteGroupKey).map((p) => ({
          ...
          底盘型号: p.底盘型号,
          变速箱型号: p.变速箱型号,
        })) : []) as unknown as ComponentProps<typeof VehicleDeleteModal>["vehicles"]}
      ...
      <VehiclePriceViewModal
        ...
        vehicles={viewVehicles.map((p) => ({
          ...
          底盘型号: p.底盘型号,
          变速箱型号: p.变速箱型号,
        })) as unknown as ComponentProps<typeof VehiclePriceViewModal>["vehicles"]}
      />
    </div>
  );
}
