...
/* 其它收支收款方式 — 客户端交互组件
 * 首屏数据由服务端 page.tsx 查询后通过 initialOperators/initialMethods 传入，
 * 增删改/排序后仍走客户端 loadData 刷新 */

export default function PaymentMethodsContent({
  initialOperators,
  initialMethods,
}: {
  initialOperators: 操作员[];
  initialMethods: 收款方式[];
}) {
  const [operators, setOperators] = useState<操作员[]>(initialOperators);
  const [methods, setMethods] = useState<收款方式[]>(initialMethods);
