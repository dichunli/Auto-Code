...
/* 工具管理 — 客户端交互组件
 * 首屏数据（工具列表/知识库标题/未归还记录/管理员身份）由服务端 page.tsx 查询后传入，
 * 搜索、借还操作后仍走客户端 加载数据 刷新 */
export default function ToolManagementContent({
  initialTools,
  initialKnowledgeEntries,
  initialUnreturnedRecords,
  initialIsAdmin,
}: {
  initialTools: 工具[];
  /* Map 不便跨服务端/客户端边界传递，用 [id, title] 元组数组传入后重建 */
  initialKnowledgeEntries: [string, string][];
  initialUnreturnedRecords: 借用记录[];
  initialIsAdmin: boolean;
}) {
  const supabase = useMemo(() => createClient(), []);
  const [工具列表, set工具列表] = useState<工具[]>(initialTools);
  const [知识库映射, set知识库映射] = useState<Map<string, string>>(() => new Map(initialKnowledgeEntries));
  const [未归还记录, set未归还记录] = useState<Map<string, 借用记录>>(
    () => new Map(initialUnreturnedRecords.map((r) => [r.tool_id, r]))
  );
  ...
  const [是管理员, set是管理员] = useState(initialIsAdmin);
  ...
  /* 首屏数据已由服务端传入，跳过挂载时的首次加载；
   * 之后搜索词变化（防抖后 加载数据 引用变化）或借还完成时再加载 */
  const 跳过首屏 = useRef(true);
  useEffect(() => {
    if (跳过首屏.current) {
      跳过首屏.current = false;
      return;
