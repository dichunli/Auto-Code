...
export default function EditServiceNamePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [{ data: name }, { data: cats }, { data: partLinks }] = await Promise.all([
          supabase.from("service_names").select("*").eq("id", id).single(),
          supabase
            ...
            .order("name"),
          supabase.from("service_name_part_names").select("part_name_id, sort_order, quantity").eq("service_name_id", id).order("sort_order", { ascending: true }),
        ]);
        ...
        setForm({
          ...
          sales_type: name.sales_commission_type || "",
          sales_value: name.sales_commission_value?.toString() || "",
          diagnosis_type: name.diagnosis_commission_type || "",
          diagnosis_value: name.diagnosis_commission_value?.toString() || "",
          repair_type: name.repair_commission_type || "",
          repair_value: name.repair_commission_value?.toString() || "",
          qc_type: name.qc_commission_type || "",
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 max-w-2xl space-y-4">
        ...
        <div className="flex gap-3 justify-end pt-4">
          <button
            type="button"
            onClick={() => router.push("/service-names")}
