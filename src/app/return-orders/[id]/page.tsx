...
export default async function ReturnOrderDetailPage({
  ...
  const { data: order } = await supabase
    ...
    .select(
      "id, return_no, supplier_name, total_quantity, status, logistics_company, tracking_no, return_shipping_fee, shipping_fee_payer, freight_recorded, goods_photos, package_photos, handover_photos, notes, created_at, profiles(full_name)"
    )
    .eq("id", id)
    .single();
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-6 print:border-black print:rounded-none">
        ...
        <div className="px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          {/* 退货照片（2026-09-18）：货物照 + 外包装照 + 交接照，确认退货时必填、详情可查 */}
          {((returnOrder.goods_photos && returnOrder.goods_photos.length > 0) ||
            (returnOrder.package_photos && returnOrder.package_photos.length > 0) ||
            (returnOrder.handover_photos && returnOrder.handover_photos.length > 0)) && (
            <div className="col-span-2 md:col-span-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              {([
                ["货物照片", returnOrder.goods_photos],
                ["外包装照片", returnOrder.package_photos],
                ["交接照片", returnOrder.handover_photos],
                    ...
                    <div className="flex flex-wrap gap-2">
                      {urls.map((url, i) => (
                        <a key={i} href={url} target="_blank" rel="noopener noreferrer">
                          <img src={url} alt={label} loading="lazy" className="w-20 h-20 object-cover rounded border border-gray-200 hover:opacity-80" />
                        </a>
                      ))}
                    </div>
                  </div>
