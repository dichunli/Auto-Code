...
export default function PartNamesContent({ initialPartNames, initialCategories }: { initialPartNames: unknown[]; initialCategories: unknown[] }) {
  ...
  async function handleImportFile(file: File) {
    ...
    try {
      ...
      const { data: categoriesData } = await supabase.from("part_categories").select("id, name");
      const categoryMap = new Map((categoriesData as PartCategory[] || []).map((c) => [c.name, c.id]));
      ...
      /* 分批插入走 Server Action（中途 session 断不会留下半截导入） */
      const 导入结果 = await 批量导入配件名称({ rows: toInsert });
      if (!导入结果.success) {
