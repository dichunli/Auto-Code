...
/* 采购分析报表 — 客户端交互组件
 * 首屏数据（无日期筛选）由服务端 page.tsx 查询后传入，
 * 选择日期范围点「查询」后仍走客户端 loadData */
export default function ProcurementReportContent({
  initialInboundOrders,
  initialReturnOrders,
  initialInboundItems,
}: {
  initialInboundOrders: InboundOrder[];
  initialReturnOrders: ReturnOrder[];
  initialInboundItems: InboundItem[];
}) {
  const supabase = useMemo(() => createClient(), []);
  const [inboundOrders, setInboundOrders] = useState<InboundOrder[]>(initialInboundOrders);
  const [returnOrders, setReturnOrders] = useState<ReturnOrder[]>(initialReturnOrders);
  const [inboundItems, setInboundItems] = useState<InboundItem[]>(initialInboundItems);
