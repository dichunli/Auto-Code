...
export default function ArrivalsContent({ initialList, initialCount }: { initialList: 到货单[]; initialCount: number }) {
  const supabase = useMemo(() => createClient(), []);
  const [列表, set列表] = useState<到货单[]>(initialList);
  /* 分页状态：首屏数据由服务端给（第 1 页），翻页走 load列表 */
  ...
  async function load列表(目标页: number) {
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {列表.length > 0 ? (
          ...
          <div className="p-12 text-center text-gray-400">
            {loading ? "加载中..." : "暂无到货确认单，点右上角「新建到货单」开始验货"}
          </div>
        )}
      </div>
