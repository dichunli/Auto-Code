...
export default function OrdersContent({ initialOrders, initialCount }: { initialOrders: 采购订单[]; initialCount: number }) {
  const supabase = useMemo(() => createClient(), []);
  const [orders, setOrders] = useState<采购订单[]>(initialOrders);
  /* 分页状态：首屏数据由服务端给（第 1 页），后续翻页走 loadOrders */
  ...
  async function loadOrders(目标页: number) {
