...
export default function MaterialReturnsContent({ initialRecords }: { initialRecords: 退料单[] }) {
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (kw) {
      list = list.filter(
        (o) =>
          o.return_no.includes(kw) ||
          (o.work_orders?.order_no || "").includes(kw) ||
          (o.picking_orders?.picking_no || "").includes(kw)
      );
    }
    if (dateFrom) {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left font-medium text-gray-500">退料单号</th>
                  <th className="px-6 py-3 text-left font-medium text-gray-500">关联工单</th>
                  <th className="px-6 py-3 text-left font-medium text-gray-500">关联领料单</th>
                ...
                {pagedOrders.map((o) => (
                  <tr key={o.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-medium text-blue-600">
                      <Link href={`/material-returns/${o.id}`} className="hover:text-blue-700">
                        {o.return_no}
                      </Link>
                    </td>
                    ...
                    <td className="px-6 py-4 text-gray-600">
                      {o.picking_orders ? (
                        <Link
                          href={`/picking-orders/${o.picking_orders.id}`}
                          className="text-blue-600 hover:text-blue-700"
                        >
                          {o.picking_orders.picking_no}
                        </Link>
                      ) : (
                        "-"
                      )}
                    ...
                    <td className="px-6 py-4 text-gray-600">
                      {(o.return_type && 退料类型标签[o.return_type]) || o.return_type || "-"}
                    </td>
