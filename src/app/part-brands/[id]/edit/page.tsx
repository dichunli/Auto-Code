...
export default function EditPartBrandPage() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: brand }, { data: links }] = await Promise.all([
        supabase.from("part_brands").select("*").eq("id", id).single(),
        supabase
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (toInsert.length > 0) {
      const rows = toInsert.map((pid) => ({ brand_id: id, part_name_id: pid }));
      const { error: insError } = await supabase.from("part_name_brands").insert(rows);
      if (insError) {
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 max-w-2xl">
        ...
        <div className="mt-8 flex gap-3 justify-end">
          <button
            type="button"
            onClick={() => router.push("/part-brands")}
