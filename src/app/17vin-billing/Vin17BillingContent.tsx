...
export default function Vin17BillingContent({ 首屏 }: { 首屏: 首屏数据 }) {
  const [余额, set余额] = useState<余额信息 | null>(null);
  const [余额加载中, set余额加载中] = useState(false);
  const [余额错误, set余额错误] = useState<string | null>(null);

  /* 首屏调用记录由服务端 page.tsx 查询后传入，翻页时才在客户端重新加载 */
  const [记录列表, set记录列表] = useState<调用记录[]>(首屏.记录列表);
  const [记录总数, set记录总数] = useState(首屏.记录总数);
  const [当前页, set当前页] = useState(1);
  const [记录加载中, set记录加载中] = useState(false);
  const [记录错误, set记录错误] = useState<string | null>(首屏.错误);
  ...
  /* 初始加载：只查余额（调用记录首屏已由服务端传入） */
  useEffect(() => {
    查询余额();
  }, [查询余额]);

  /* 页码变化时重新加载（跳过首屏——首屏数据已由服务端传入） */
