...
export default async function PartCategoriesPage() {
  ...
  const { data: categories } = await supabase.from("part_categories").select("*").order("created_at", { ascending: false });
  ...
  return (
    <div>
      <PageHeader title="配件分类" description="管理分类、耗材属性及各类提成标准" action={{ href: "/part-categories/new", label: "新建分类" }} />
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {(!categories || categories.length === 0) && (
                <tr><td colSpan={8} className="px-6 py-12 text-center text-gray-400">暂无分类，请先新建</td></tr>
              )}
            </tbody>
          </table>
        </div>
