...
function CustomToolbarButtons({
  ...
  const imageInputId = `bn-img-${useId()}`;
  const videoInputId = `bn-video-${useId()}`;
  const fileInputId = `bn-file-${useId()}`;
  ...
  return (
    <>
      ...
      {是Capacitor环境() ? (
        ...
        <label htmlFor={imageInputId} className={btnBase + " cursor-pointer"} title="插入图片">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          图片
      ...
      <input id={imageInputId} ref={imageInputRef} type="file" accept="image/*" className="hidden" onChange={handleInsertImage} />
      ...
      {/* 上传视频 */}
      <label htmlFor={videoInputId} className={btnBase + " cursor-pointer"} title="上传视频">
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z" />
        </svg>
        上传视频
      </label>
      <input id={videoInputId} ref={videoFileInputRef} type="file" accept="video/*" className="hidden" onChange={handleUploadVideo} />

      {/* 上传文件 */}
      <label htmlFor={fileInputId} className={btnBase + " cursor-pointer"} title="上传文件">
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
        文件
      </label>
      <input id={fileInputId} ref={fileInputRef} type="file" accept=".doc,.docx,.xls,.xlsx,.ppt,.pptx,.pdf" className="hidden" onChange={handleUploadOfficeFile} />
