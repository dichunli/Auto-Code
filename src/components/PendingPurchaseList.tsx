...
function 暂存行转展示行(
  s: {
    id: string; part_id: string | null; part_number: string | null; name: string;
    brand: string | null; specification: string | null; document_name: string | null;
    unit: string | null; unit_cost: number | null; quantity: number;
  ...
  return {
    ...
    parts: 库存数量 != null ? { quantity: 库存数量 } : null,
...
interface PendingPurchaseListProps {
  ...
  /* 服务端首屏工单配件行的总条数（两阶段分页的 count，不含自定义暂存行），有 initialRows 时必传 */
...
export function PendingPurchaseList(props: PendingPurchaseListProps) {
  ...
  /* 分页（2026-09-15）：两阶段数据库分页只作用于工单配件行；
     自定义采购暂存行（custom_purchase_staging）不参与分页，照旧全量拼在列表尾部 */
  ...
  const loadData = useCallback(async (目标页 = 1) => {
    setLoading(true);
    /* 阶段1：轻量取"符合待采购"的工单配件行 id 集合（只取 id 一列）。
       与原"DB 预过滤 + 内存谓词 行符合待采购"严格同口径：
       customer_opinion=agree、is_purchased=false（原 DB 预过滤），
       门禁=工单未结算 + 非作废/保养单，价格=进价>0 且销售价>0（原内存谓词，NULL 被 gt 排除同口径），
       库存=无关联配件直接合格 / 关联配件且库存 ≤0 合格（有库存 >0 才不采购）。
       1a=无库存关联；1b=关联且库存 ≤0（parts!inner 内连接，part_id 有外键约束不会悬空） */
    const [
      { data: 无关联行, error: 无关联错误 },
      { data: 有关联行, error: 有关联错误 },
      { data: sups },
      { data: logistics },
      { data: stagingData },
    ] = await Promise.all([
    ...
    if (无关联错误 || 有关联错误) {
      console.error("加载待采购 id 集合失败:", 无关联错误 || 有关联错误);
      setLoading(false);
      return;
    }
    const 合格ids = [
    ...
    const 暂存行列表: PartBranchRow[] = ((stagingData || []) as unknown as 暂存行[]).map((s) => {
      const p = Array.isArray(s.parts) ? s.parts[0] : s.parts;
      return 暂存行转展示行(s, p?.quantity ?? null);
    });
  ...
  const 配件联动 = usePartLinking<PartBranchRow>({
    ...
    /* 局部更新：行内选中/清除配件后该行的库存 join 和"为空才填"合并结果
       前端都不知道（可能因有库存而离开待采购），只重查这一行 */
    保存后: (rowId) => { void 重查单行(rowId); },
  });
  const {
    editRow,
    editId,
  ...
  async function 保存数量(row: PartBranchRow) {
    ...
    try {
      ...
      set数量草稿((prev) => { const n = { ...prev }; delete n[row.id]; return n; });
      /* 局部更新：数量是前端已知值，直接 patch 该行，不整表重查 */
      setRows((prev) => 更新列表项(prev, row.id, { quantity: 新值 }));
    } catch (err: unknown) {
  ...
  async function handleCreatePurchases(forcedLogisticsId?: string | null) {
    ...
    try {
      ...
      setSelected(new Set());
      /* 局部更新：发起采购后工单行 is_purchased=true、暂存行被事务删除，
         都必然离开待采购列表，直接移除，不整表重查；
         总数只扣工单配件行（暂存行不参与分页计数） */
      const 已采购ids = new Set(selectedRows.map((r) => r.id));
      setRows((prev) => prev.filter((r) => !已采购ids.has(r.id)));
      setTotalCount((c) => Math.max(0, c - 工单行.length));
    } catch (err: unknown) {
  ...
  async function handleCreateStockPurchases() {
    ...
    try {
      ...
      /* 局部更新：action 已返回插入的暂存行，直接 append 到列表末尾；
         库存数量本地就有（lowStockParts 弹窗数据），不用再查 */
      if (res.rows && res.rows.length > 0) {
        const 库存Map = new Map(selectedParts.map((p) => [p.id, p.quantity]));
        const 新行们 = res.rows.map((s) =>
          暂存行转展示行(s, s.part_id ? 库存Map.get(s.part_id) ?? null : null)
        );
        setRows((prev) => [...prev, ...新行们]);
      }
    } catch (err: unknown) {
      const e = err as Error;
  ...
  async function handleOpinionChange(row: PartBranchRow, 意见: string) {
    ...
    try {
      ...
      /* 局部更新：改"未确定"退回待确认、改"否决"不再显示，
         该行必然离开待采购列表，直接移除，不整表重查（总数同步 -1；
         客户意见选择器只有工单配件行有，暂存行走不到这里） */
  ...
  async function handleRevoke() {
    ...
    try {
      ...
      /* 局部更新：撤销后客户意见变为未确定/否决，行必然离开待采购，直接移除；
         总数只扣工单配件行（暂存行不参与分页计数） */
      const 撤销ids = new Set(ids);
      setRows((prev) => prev.filter((r) => !撤销ids.has(r.id)));
      setTotalCount((c) => Math.max(0, c - selectedRows.filter((r) => !r.staging).length));
    } catch (err: unknown) {
      const e = err as Error;
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                        ...
                        <td className="px-3 py-3 text-gray-700 whitespace-nowrap">
                          ...
                          {r.staging ? (
                            ...
                            <DocumentNameInput
                              ...
                              初始值={r.document_name || ""}
                              保存后={(新值) =>
                                /* 局部更新：单据名称是前端已知值，直接 patch 该行 */
                                setRows((prev) => 更新列表项(prev, r.id, { document_name: 新值 || null }))
                              }
                            />
                          )}
                        </td>
      ...
      {/* 翻页：只分页工单配件行；下方弹窗都是 fixed 不占文档流，视觉上就在列表最底部 */}
      <Pagination page={page} totalCount={totalCount} pageSize={每页条数} onChange={(p) => loadData(p)} />

      {确认弹窗}

      {/* 自定义采购弹窗：采购与工单无关的配件；添加成功走 on新增 局部 append（不再整表重查） */}
      <CustomPurchaseModal
        ...
        on新增={async (新暂存行) => {
          if (新暂存行.length === 0) return;
          /* 库存数量影响"有库存不显示"的展示，对关联了库存配件的行补查一次 */
          const 关联ids = 新暂存行.map((s) => s.part_id).filter((x): x is string => !!x);
          let 库存Map = new Map<string, number | null>();
          if (关联ids.length > 0) {
            const { data: 库存行 } = await supabase.from("parts").select("id, quantity").in("id", 关联ids);
            库存Map = new Map(((库存行 || []) as { id: string; quantity: number | null }[]).map((p) => [p.id, p.quantity]));
          }
          const 新行们 = 新暂存行.map((s) =>
            暂存行转展示行(s, s.part_id ? 库存Map.get(s.part_id) ?? null : null)
          );
          setRows((prev) => [...prev, ...新行们]);
        }}
