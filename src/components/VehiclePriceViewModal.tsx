...
interface VehiclePriceViewModalProps {
  open: boolean;
  onClose: () => void;
  onDeleteVehicles?: (vehicleIds: number[]) => void;
  vehicles: VehicleItem[];
}

export default function VehiclePriceViewModal({ open, onClose, onDeleteVehicles, vehicles }: VehiclePriceViewModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <h3 className="text-base font-semibold text-gray-900">查看关联车型</h3>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
                ...
                <tr key={v.vehicle_model_id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 text-center">
                    {onDeleteVehicles && (
                      <button
                        type="button"
                        onClick={() => handleDelete(v.vehicle_model_id)}
              ...
              {displayVehicles.length === 0 && (
                <tr>
                  <td colSpan={11} className="px-3 py-8 text-center text-gray-400">无匹配车型</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="px-6 py-3 border-t border-gray-200 flex items-center justify-between bg-gray-50">
          <div className="flex items-center gap-4">
            ...
            {onDeleteVehicles && selectedIds.size > 0 && (
