...
import { ReceiptOrderCard, 行可收货 } from "./ReceiptOrderCard";
...
export function PendingReceiptList(props: PendingReceiptListProps) {
  ...
  /* orderNeedsWaybill/行可收货/getReceiptStatus 随卡片搬到 ReceiptOrderCard（2026-09-16），
     这里 import 回来用（口径仍只有一份） */
...
{g.orders.map((order) => (
              <ReceiptOrderCard
                ...
                handleRevokeItem={handleRevokeItem}
                handleUnstage={handleUnstage}
                openReceiveModal={openReceiveModal}
                openGateModal={openGateModal}
                handleRevokeItemToPending={handleRevokeItemToPending}
                handleDiscardItem={handleDiscardItem}
                openEditModal={openEditModal}
                openCreateNewModal={openCreateNewModal}
                handleInlinePartSelect={handleInlinePartSelect}
                handleInlineClear={handleInlineClear}
                patch明细={patch明细}
              />
            ))}
          </div>
