/* ============================================================
 * 管钱/开单 RPC 补权限门禁（2026-09-19，9-15 诊断🟠#6 深挖）
 *
 * 背景：补结算拒绝路径测试时发现 settle_work_order / create_work_order
 *   函数体内【完全没有 auth.uid() / 角色校验】——同场景的预收款函数
 *   （register_advance_payment，2026-08-27 起）已有 admin/boss/receptionist/accountant
 *   门禁且生产运转至今，本次把结算函数对齐同一口径；开单函数只加登录校验
 *   （开单角色口径未拍板，不擅自收窄）。
 *
 * 门禁口径：结算 = admin/boss/receptionist/accountant（对齐预收款现有口径）。
 * 幂等：CREATE OR REPLACE（参数列表未变，无需 DROP）；
 *   ⚠️ 必须带 SET search_path = public——OR REPLACE 不带会丢 0902 迁移设的属性。
 * ============================================================ */

/* ─── 一、settle_work_order：函数体与 20260501000002 版完全一致，仅开头插门禁 ─── */
...
/* ─── 二、create_work_order（13 参现行版）：函数体与 20260523 版一致，仅开头加登录校验 ───
 * 不加角色门禁：开单角色口径未拍板（接待开单为主，不排除技师代开），先只挡匿名。 */
