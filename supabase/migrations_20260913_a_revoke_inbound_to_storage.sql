/* ═══════════════════════════════════════════════════════════
 * 2026-09-13 撤销已入库改为「退回待入库」（用户拍板）
 *
 * 背景：老版 revoke_completed_inbound（2026-08-16）是一路退到底——
 *   清空收货处理结果(handle_action/received_qty)、回退到货标记、
 *   删待退货记录、采购单回 submitted（待收货）。
 *   用户拍板：撤销"入库"应该只倒退一步到「待入库」，
 *   收货结果全部保留（库存扣回、入库单删除即可）。
 *
 * 新语义（本文件 CREATE OR REPLACE 同签名替换，action 层不动）：
 *   ┌ 蓝卡流程（明细未挂批次）：单单回滚
 *   └ 黄卡流程（明细挂了批次）：整批回滚——批次入库单是批次级一张单，
 *     批次下所有采购单一并退回 pending_storage，批次回 pending_storage
 *   共同保留：handle_action/received_qty/is_arrived/待退货记录/补货分支/
 *            明细 receiving_batch_id（黄卡重新显示）
 *   共同回滚：总库存+仓位（净额对称）、入库单(含未确认的 draft)/
 *            库存批次/库存流水/应付款
 * ═══════════════════════════════════════════════════════════ */
...
/* 函数注释同步更新（原注释还是"退到待收货"的老口径） */
COMMENT ON FUNCTION revoke_completed_inbound(UUID, UUID) IS
  '撤销已入库→退回待入库（2026-09-13 改）：库存/入库单/流水/应付款回滚，收货结果保留；蓝卡单单回滚、黄卡整批回滚';

/* 权限保持原样（函数体重建不影响 GRANT，这里兜底再声明一次） */
