/* ============================================================
   修复 check_promotion_eligibility 函数的枚举值错误

   问题：函数统计"员工完工工单数"时引用了不存在的工单状态
   'completed'，导致函数执行报错（22P02），
   晋级总览页所有员工工单数显示 0、晋级资格全部不合格。

   修复：'completed' 替换为真实存在的"已完工"类存储状态
   （待质检、待结单、待结算、已结算、已交车）。

   说明：函数体与 migrations_20260605 版本完全一致，
   仅第 92 行状态列表不同；同时保留 search_path 设置
   （migrations_20260802 的纯声明修复）。
   ============================================================ */
...
BEGIN
  ...
  WHERE woi.mechanic_id = p_employee_id
    ...
    AND wo.status IN ('pending_quality_check', 'pending_close', 'pending_settlement', 'settled', 'delivered');
