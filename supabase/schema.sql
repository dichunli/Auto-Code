-- ============================================================
-- ⚠️ 已废弃（2026-08-19 标注）：本文件严重滞后于生产库，仅作历史参考。
-- 采购/入库/退货/询价等 2026-05 后新增的表和函数均未收录，
-- 采购单状态约束等还是旧版。真实结构以 supabase/migrations_*.sql
-- 按日期顺序的执行结果为准。禁止按本文件重建数据库！
-- ============================================================
--
...
CREATE TABLE part_categories (
  ...
  auto_match_17vin_models BOOLEAN DEFAULT FALSE,
...
CREATE TABLE part_names (
  ...
  auto_match_17vin_models BOOLEAN DEFAULT FALSE,
  is_consumable BOOLEAN DEFAULT FALSE,
  require_scan_check BOOLEAN DEFAULT FALSE,
  require_location_check BOOLEAN DEFAULT FALSE,
...
/* ════════════════════════════════════════════════════════════════════════════
 * ⚠️⚠️⚠️ 警告：以下 69 条 auth_full_access 策略是 2026-06 建库时的历史快照 ⚠️⚠️⚠️
 *
 * 这些"登录即全权"策略【早已和生产库不一致】——生产库的 profiles/employee_groups
 * 等敏感表策略已在后续迁移中逐批收紧（如 profiles UPDATE 仅 is_admin()，
 * 见 migrations_20260802_finance_role_rls.sql 及之后各 rls 迁移）。
 *
 * 禁止用本文件重建数据库或给 CI 建库——否则权限"拒绝路径"测试会全部失效，
 * 重建出的库等于全员裸奔。重建库唯一正确方式：按字母序重放 migrations_*.sql。
