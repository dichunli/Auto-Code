...
-- 日期：2026-07-29
--
-- 1. add_construction_log 改写：
--    a. start/resume 补"客户同意才能开工"门禁（此前改写时丢失，UI 层以外无兜底）
--    b. 工时统计（construction_stats）搬进 RPC：start/resume 建行、complete 结算秒数、
--       cancel 删除/恢复——根治"页面关了统计就丢"的老毛病，详情页与列表卡片共用一份逻辑
-- 2. fn_order_ready_to_close：labor 统计排除客户否决(reject)的项目（不做的项目不阻塞结单）
-- 3. submit_item_qc："全部合格自动待结单"判定同步排除 reject 项目
-- ============================================================
...
-- ── 2. fn_order_ready_to_close：labor 统计排除客户否决(reject)的项目 ──
...
-- ── 3. submit_item_qc："全部合格自动待结单"判定排除 reject 项目 ──
...
BEGIN
  ...
  IF p_result = 'passed' THEN
    ...
    -- 规则7：全部 labor 完工且须质检全部合格 → 工单自动待结单（否决项目不参与）
    IF NOT EXISTS (
      SELECT 1 FROM work_order_items
