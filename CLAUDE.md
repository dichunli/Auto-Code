...
### 交互与错误处理
- **禁用浏览器原生弹窗（`alert()` / `confirm()` / `prompt()`）**，一律用自定义 React 弹窗（2026-09-11 用户拍板）：
  - 操作确认（删除、作废、出库等）→ `useConfirm()`（`src/components/ConfirmDialog.tsx`）：`if (!(await 请求确认("确定删除吗？"))) return;` + JSX 渲染 `{确认弹窗}`
  - 必须让用户看完的重要提示（长文本）→ `useAlert()`（同文件）：`await 请求提示(...)` + `{提示弹窗}`
  - 轻量成功/失败提示 → `useToast()`（`showToast("...", "error")`）
- 删除操作前必须弹确认（`useConfirm`）
...
- **迁移文件三防（2026-09-13 起，防"重跑报错/函数残留/台账漏登"）**：
  - 建表建索引用 `IF NOT EXISTS`，改函数用 `CREATE OR REPLACE`，保证重跑无害
  - **改函数参数列表时必须先 `DROP FUNCTION` 再 `CREATE`**（`CREATE OR REPLACE` 改参数列表会生成重载残留，曾致建到货单接口报错事故）
  - 台账插入统一 `INSERT INTO migration_log (file_name) VALUES ('文件名') ON CONFLICT DO NOTHING;`
  - 部署前跑 `npm run db:check` 自查"文件写了没执行"
...
- **敏感操作必须二次确认**：金额修改、批量删除、数据导出等操作必须用 `useConfirm()` 专用确认弹窗，不能一触即发
...
- **涉及金额、库存的 Server Action / RPC 必须有集成测试**：管钱管货的代码没有测试保护出过事故（并发入库丢库存），数据库测试写法参照 `supabase/tests/settle-work-order.test.ts`
- **权限校验必须有"拒绝路径"测试**：未登录、角色不足时必须断言被拦住，不能只测正常路径
- **修 bug 必须补回归测试**：每个修复提交附一条能复现该 bug 的测试，防止复发
- **数据库集成测试**：放 `supabase/tests/`，用 `npm run test:db` 单独跑（需本地 Postgres）；CI 里由 GitHub Actions 自动建库执行
